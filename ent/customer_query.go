...
type CustomerQuery struct {
	...
	withFollows      *AccountQuery
	withItems        *ItemQuery
	modifiers        []func(*sql.Selector)
	loadTotal        []func(context.Context, []*Customer) error
	withNamedFollows map[string]*AccountQuery
...
func (cq *CustomerQuery) QueryFollows() *AccountQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, false, customer.FollowsTable, customer.FollowsPrimaryKey...),
		)
...
func (cq *CustomerQuery) QueryItems() *ItemQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, customer.ItemsTable, customer.ItemsColumn),
		)
		fromU = sqlgraph.SetNeighbors(cq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (cq *CustomerQuery) WithFollows(opts ...func(*AccountQuery)) *CustomerQuery {
	query := (&AccountClient{config: cq.config}).Query()
...
func (cq *CustomerQuery) WithItems(opts ...func(*ItemQuery)) *CustomerQuery {
	query := (&ItemClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Customer, error) {
	...
	if query := cq.withFollows; query != nil {
		if err := cq.loadFollows(ctx, query, nodes,
			func(n *Customer) { n.Edges.Follows = []*Account{} },
			func(n *Customer, e *Account) { n.Edges.Follows = append(n.Edges.Follows, e) }); err != nil {
			return nil, err
		}
	}
	if query := cq.withItems; query != nil {
		if err := cq.loadItems(ctx, query, nodes,
			func(n *Customer) { n.Edges.Items = []*Item{} },
			func(n *Customer, e *Item) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedFollows {
		if err := cq.loadFollows(ctx, query, nodes,
			func(n *Customer) { n.appendNamedFollows(name) },
			func(n *Customer, e *Account) { n.appendNamedFollows(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedItems {
		if err := cq.loadItems(ctx, query, nodes,
			func(n *Customer) { n.appendNamedItems(name) },
			func(n *Customer, e *Item) { n.appendNamedItems(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CustomerQuery) loadFollows(ctx context.Context, query *AccountQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Account)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(customer.FollowsTable)
		s.Join(joinT).On(s.C(account.FieldID), joinT.C(customer.FollowsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(customer.FollowsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(customer.FollowsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "follows" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (cq *CustomerQuery) loadItems(ctx context.Context, query *ItemQuery, nodes []*Customer, init func(*Customer), assign func(*Customer, *Item)) error {
	...
	query.Where(predicate.Item(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(customer.ItemsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.customer_items
		if fk == nil {
			return fmt.Errorf(`foreign-key "customer_items" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "customer_items" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CustomerQuery) WithNamedFollows(name string, opts ...func(*AccountQuery)) *CustomerQuery {
	query := (&AccountClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedFollows == nil {
		cq.withNamedFollows = make(map[string]*AccountQuery)
	}
	cq.withNamedFollows[name] = query
	return cq
}

// WithNamedItems tells the query-builder to eager-load the nodes that are connected to the "items"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (cq *CustomerQuery) WithNamedItems(name string, opts ...func(*ItemQuery)) *CustomerQuery {
	query := (&ItemClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if cq.withNamedItems == nil {
		cq.withNamedItems = make(map[string]*ItemQuery)
	}
	cq.withNamedItems[name] = query
	return cq
}
